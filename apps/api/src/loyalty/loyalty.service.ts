...
export class LoyaltyService {
  ...
  /** Otorga puntos a un usuario por un pedido pagado. */
  async awardForOrder(userId: string | null | undefined, totalUsd: number) {
    if (!userId) return;
    const points = pointsForOrder(totalUsd);
    if (points <= 0) return;
    await this.prisma.user.update({ where: { id: userId }, data: { points: { increment: points } } });
  ...
  async redeemForOrder(userId: string | null | undefined, points: number) {
    ...
    await this.prisma.user.update({ where: { id: userId }, data: { points: { decrement: points } } });
